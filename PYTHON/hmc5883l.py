...
class HMC5883L:
    ...
    OUTPUT_RATE = {0: 0.75, 4: 1.5, 8: 3,
                   12: 7.5, 16: 15, 20: 30, 24: 75, 28: 0}
    BIAS_MODE = {0: "normal", 1: "pos_bias", 2: "neg_bias"}
    GAIN = {0: 1370, 32: 1090, 64: 820, 96: 660,
            128: 440, 160: 390, 192: 330, 224: 230}
    RESOLUTION = {0: 0.73, 32: 0.92, 64: 1.22, 96: 1.52,
                  128: 2.27, 160: 2.56, 192: 3.03, 224: 4.35}
    ...
    def get_field_x(self):
        _x = self.bus.read_byte_data(self.DEV_ADDR, self.REG_X_MSB) << 8 | self.bus.read_byte_data(
            self.DEV_ADDR, self.REG_X_LSB)
        _x = self._two_comp_to_dec(_x)
        return _x*self.meas_res

    def get_field_y(self):
        _y = self.bus.read_byte_data(self.DEV_ADDR, self.REG_Y_MSB) << 8 | self.bus.read_byte_data(
            self.DEV_ADDR, self.REG_Y_LSB)
        _y = self._two_comp_to_dec(_y)
        return _y*self.meas_res

    def get_field_z(self):
        _z = self.bus.read_byte_data(self.DEV_ADDR, self.REG_Z_MSB) << 8 | self.bus.read_byte_data(
            self.DEV_ADDR, self.REG_Z_LSB)
